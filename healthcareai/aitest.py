...
patients = '..//march_madness/aistats.csv'
df = pandas.read_csv(patients)
targets = []
features = []
df.drop(['AdmitDTS', 'AdmitTypeCD', 'GenderCD'], axis=1, inplace=True)
columns = df.columns.tolist()
columns = [c for c in columns if c not in ["LOSFLG"]]
target = "LOSFLG"
train = df.sample(frac=0.1, random_state=random.seed())
