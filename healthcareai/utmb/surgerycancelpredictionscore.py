...
def main():
    ...
    #
    df = pd.read_sql(
            sql="""select CaseID, SchedulingStatusCD,SchedulingStatusDSC,PredictedProbNBR, Predicted, Actual--,ScheduledDTS,SurgeryDTS,CancelDTS,CancelReasonDSC
            from (select h.CaseID, h.PredictedProbNBR, h.LastLoadDTS, s.SchedulingStatusCD, SchedulingStatusDSC,
            CASE WHEN s.SchedulingStatusDSC = 'Canceled' AND s.CancelDTS IS NOT NULL THEN 1
            WHEN s.SchedulingStatusDSC = 'Completed' AND s.CancelDTS IS NULL THEN 0 ELSE NULL END AS Actual,
            CASE WHEN PredictedProbNBR <= 0.20 THEN 1 ELSE 0 END AS Predicted, ScheduledDTS,SurgeryDTS,CancelDTS,CancelReasonDSC,
            ROW_NUMBER() over(partition by h.CaseID order by h.LastLoadDTS desc) as rn
            from [SAM].[dbo].[HCPyDeployClassificationSurgeryCancelBASE] h
            join epic.surgery.SurgicalCaseBASE s on h.CaseID = s.CaseID) as r
            where r.rn = 1
    ...
    auc = GenerateAUC(df['Predicted'], df['Actual'], aucType='SS', plotFlg=True, allCutoffsFlg=False)
