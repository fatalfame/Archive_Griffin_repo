...
def main():
    ...
    #
    df = pd.read_sql(
        sql="""select PatientEncounterID, PredictedProbNBR, LOS, Predicted, Actual
    from (select h.patientEncounterID, h.PredictedProbNBR, h.LastLoadDTS,
    DATEDIFF(HOUR, f.InpatientAdmitDTS, f.DischargeDTS) AS LOS,
    CASE WHEN DATEDIFF(HOUR, f.InpatientAdmitDTS, f.DischargeDTS) >= 120 THEN 1 ELSE 0 END AS Actual,
    CASE WHEN PredictedProbNBR >= 0.5 THEN 1 ELSE 0 END AS Predicted,
    ROW_NUMBER() over(partition by h.patientencounterid order by h.LastLoadDTS desc) as rn
    from [SAM].[dbo].[HCPyDeployClassificationFinalBASE] h
    left join epic.Encounter.PatientEncounterBASE p on h.patientencounterID = p.PatientEncounterID
    join Shared.Clinical.FacilityAccount f on p.HospitalAccountID = f.FacilityAccountID) as r
    where r.rn = 1
