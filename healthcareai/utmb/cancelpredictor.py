...
def main():
    ...
    sc.PatientID, ScheduledDTS, 1 AS CancelCNT FROM Epic.Surgery.SurgicalCase AS SC
    WHERE 1 = 1 AND sc.CaseTypeCD = 4 and sc.SchedulingStatusCD = 2 and sc.CancelReasonCD <> 16
    and sc.surgerydts > '2015-06-01') ,y as (SELECT DISTINCT SC.CaseID, sc.PatientID ,CASE
    WHEN SC.CancelReasonDSC IS NOT NULL AND SchedulingStatusCD = 2 THEN SC.CancelReasonDSC ELSE NULL
    END AS CancelReasonDSC ,CASE WHEN SchedulingStatusDSC = 'Canceled' THEN SC.CancelDTS ELSE NULL
    END AS CancelDTS, sc.SchedulingStatusDSC ,CASE WHEN SC.CancelDTS IS NOT NULL AND SchedulingStatusCD = 2
    AND CONCAT(DATEPART(MM,SC.CancelDTS),DATEPART(DD,SC.CancelDTS)) =
    CONCAT(DATEPART(MM,SC.SurgeryDTS),DATEPART(DD,SC.SurgeryDTS)) THEN 1 ELSE 0 END AS SameDayCancelFLG,
    ...
    sc.PatientAgeNBR, p.SexCD, sc.CaseTypeCD, sc.CaseClassCD, sc.PatientClassCD, sc.ServiceCD,
    sc.TotalTimeNeededMinutesNBR, sc.LocationID, sc.AddOnCaseFLG, sc.PatientLevelCD, p.PrimaryFinancialClassCD,
    sc.surgeryDTS, sc.schedulingstatuscd, p.EmploymentStatusCD, p.StateCD, sp.ProcedureID, sp.BodyRegionCD,
    sc.ScheduledDTS, CASE WHEN SchedulingStatusCD = 2 and CancelDTS IS NOT NULL THEN 1
    WHEN sc.SchedulingStatusCD IN (1, 3, 8) AND SurgeryDTS < GETDATE() THEN 0 ELSE NULL END AS CancelFLG
    FROM Epic.Surgery.SurgicalCase AS SC LEFT JOIN epic.Surgery.Room AS r ON sc.OperatingRoomID = r.ProviderID
    ...
    WHERE 1 = 1 and sc.SurgeryDTS > '2015-06-01' AND sc.CaseTypeCD = 4 and sc.SchedulingStatusDSC NOT IN ('Voided'))
    select distinct y.CaseID, CASE WHEN y.SurgeryDTS < GETDATE() AND SchedulingStatusCD IN (2,8) THEN 'N'
    ...
    DATEPART(dw, y.SurgeryDTS) AS DayOfSurgeryNBR, y.EmploymentStatusCD, y.SameDayCancelFLG, y.PatientAge,
    y.PatientAgeGroup, y.SexCD, y.CaseClassCD, y.PatientClassCD, y.ServiceCD, y.TotalTimeNeededMinutesNBR,
    y.LocationID, CASE WHEN y.AddOnCaseFLG = 'Y' THEN 1 ELSE 0 END AS AddOnCaseFLG, y.PrimaryFinancialClassCD,
    DATEPART(mm, y.SurgeryDTS) AS MonthOfSurgeryNBR, y.BodyRegionCD from y left join x on y.patientID = x.PatientID
    WHERE SurgeryDTS < GETDATE() + 1095""", con=cnxn)
