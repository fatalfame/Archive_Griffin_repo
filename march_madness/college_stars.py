...
players = '..//march_madness/cbb_player_stats_final_final.csv'
...
def main():
    ...
    targets = []
    features = []
    feat_names = ['Points', 'Rebounds', 'Assists', 'Mins', 'Blocks', 'FGA', 'FGM', 'FG%', 'Steals', 'FTA']
    x_2016 = []
    pts = []
    ast = []
    for row in reader:
        if '1' not in row['eligible']:
            ...
            float(row['FTA'])]),
            targets.append(int(row['nba level']))
        else:
    ...
    # model = RandomForestClassifier(min_samples_leaf=10, n_estimators=50)
    ...
    # plt.ylabel(pred)
    # plt.show()
    classifier = RandomForestRegressor(min_samples_leaf=10, n_estimators=50)
    # classifier = SVC()
