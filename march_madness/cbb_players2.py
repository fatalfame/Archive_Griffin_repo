...
writer = csv.DictWriter(open('cbb_player_statsk-z.csv', 'wb'), fieldnames=fieldnames, lineterminator='\n')
writer.writeheader()


def main():
    link_list = []
    for a in itertools.product(string.ascii_lowercase[10:]):
        r = requests.get('http://www.sports-reference.com/cbb/players/' + str(a[0] + '-index.html'))
        tree = lxml.html.fromstring(r.content)
        players = tree.xpath(".//*[@id='content']/div/p")
        for p in players:
            link_list.append('http://www.sports-reference.com/' + str(p.xpath('.//a/@href')[0]))
        #     try:
        #         y = requests.get('http://www.sports-reference.com/' + str(p.xpath('.//a/@href')[0]))
        #     except CE:
        #         continue
        for link in link_list:
            rs = (grequests.get(u) for u in link)
            reqs = grequests.map(rs)
            for r in reqs:
                print r.content
            # x = lxml.html.fromstring(reqs)
            # player = x.xpath(".//a/text()")[0]
            # print player
        # s = p.xpath(".//small/text()")[0]
        # seasons = re.sub(r"\(|-.*", "", s)
        # z = html.fromstring(y.content)
